...
public sealed class SessionWorktreeAdvisorTests
{
    ...
    public SessionWorktreeAdvisorTests()
    {
        ...
        _sessions.Setup(repo => repo.GetByIdAsync(SessionId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Session(
                SessionId, MainRepo, null, "main", "Example", null,
    ...
    {
        GivenBranchOverride("legacy-label", localPath: MainRepo);

        var advice = await Build().AdviseAsync(SessionId, Ct);

        var advisory = Assert.Single(advice.Advisories);
        Assert.Equal(WorktreeAdvisoryKind.BranchNotFound, advisory.Kind);
        Assert.Contains("legacy-label", advisory.Message, StringComparison.Ordinal);
    ...
    /// is not checked out anywhere right now must stay silent. This is the dominant case in
    /// practice — a session labelled <c>main</c> in a repository that normally sits on a feature
    /// branch — and warning about it buries the genuine misconfigurations.
    /// </summary>
    [Fact]
    ...
    public async Task AdviseAsync_WorkspaceReaderThrows_StillReturnsAdvice()
    {
        _sessions.Setup(repo => repo.GetByIdAsync(SessionId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Session(
                SessionId, null, null, null, "Example", null,
                DateTimeOffset.UtcNow, DateTimeOffset.UtcNow));
        _overrides.Setup(repo => repo.GetOverrideAsync(SessionId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SessionOverride(
    ...
    {
        GivenWorktrees(
            new GitWorktree("C:/dev/example/app", MainBranch, "aaa", false, false, true, true));
