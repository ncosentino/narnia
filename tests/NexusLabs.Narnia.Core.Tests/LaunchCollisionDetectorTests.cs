...
public sealed class LaunchCollisionDetectorTests
{
    ...
    private const string Worktree = @"C:\dev\example\app-feature";
    ...
    {
        GivenLiveSession(LiveSession, MainRepo, name: "Example");

        var collisions = await Build().DetectAsync(
            [new TerminalLaunchTab(Launching, "Side Worktree Session", MainRepo)],
            Ct);
        ...
        Assert.Equal("Example", collision.OccupyingSessionName);
    ...
    public async Task DetectAsync_LiveSessionInDifferentWorktree_IsNotACollision()
    {
        ...
        var collisions = await Build().DetectAsync(
            [new TerminalLaunchTab(Launching, "Side Worktree Session", Worktree)],
            Ct);
    ...
    {
        GivenLiveSession(LiveSession, "C:/dev/example/app");
    ...
    public async Task DetectAsync_RelaunchingTheLiveSessionItself_IsNotACollision()
    {
        ...
        var collisions = await Build().DetectAsync(
            [new TerminalLaunchTab(Launching, "Example", MainRepo)],
            Ct);
    ...
    public async Task DetectAsync_TwoTabsInOneBatchSharingADirectory_IsReported()
    {
        var collisions = await Build().DetectAsync(
            [
                new TerminalLaunchTab(Launching, "Example", MainRepo),
                new TerminalLaunchTab(SecondLaunching, "Side Worktree Session", MainRepo),
            ],
            Ct);
    ...
    public async Task DetectAsync_TabWithNoDirectory_IsIgnored()
    {
        ...
        var collisions = await Build().DetectAsync(
            [new TerminalLaunchTab(Launching, "Example", null)],
            Ct);
    ...
    {
        var live = new LaunchDirectoryCollision(Launching, MainRepo, LiveSession, "Example", true);
        var pending = new LaunchDirectoryCollision(Launching, MainRepo, LiveSession, "Example", false);
