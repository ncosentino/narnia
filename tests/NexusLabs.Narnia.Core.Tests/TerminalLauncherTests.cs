...
public sealed class TerminalLauncherTests
{
    ...
    {
        var outcome = Launcher().Launch(ShellPath, "pwsh", [], TerminalWindowMode.SingleWindow);
    ...
    public void Launch_WindowsTerminalSingleWindow_StartsOneJoinedCommand()
    {
        ...
        _commandBuilder.Setup(b => b.BuildWindowCommand(ShellPath, "pwsh", tabs)).Returns("joined-command");

        var outcome = Launcher().Launch(ShellPath, "pwsh", tabs, TerminalWindowMode.SingleWindow);
    ...
    {
        _commandBuilder.Setup(b => b.FindWindowsTerminalPath()).Returns(WtPath);
        _commandBuilder.Setup(b => b.BuildNewTabSegment(ShellPath, "pwsh", It.IsAny<TerminalLaunchTab>()))
            .Returns<string, string, TerminalLaunchTab>((_, _, tab) => $"segment-{tab.SessionId}");

        var outcome = Launcher().Launch(ShellPath, "pwsh", [Tab("s1"), Tab("s2")], TerminalWindowMode.SeparateWindows);
    ...
    public void Launch_NoWindowsTerminal_FallsBackToDirectShellPerTab_WithDirectory()
    {
        ...
        _commandBuilder.Setup(b => b.BuildDirectLaunchArguments("pwsh", It.IsAny<TerminalLaunchTab>()))
            .Returns<string, TerminalLaunchTab>((_, tab) => $"direct-{tab.SessionId}");

        var outcome = Launcher().Launch(
            ShellPath, "pwsh", [Tab("s1", @"C:\one"), Tab("s2")], TerminalWindowMode.SingleWindow);
    ...
    {
        _commandBuilder.Setup(b => b.FindWindowsTerminalPath()).Returns(WtPath);
        _commandBuilder.Setup(b => b.BuildWindowCommand(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<TerminalLaunchTab>>()))
            .Returns("cmd");
        _processLauncher.Setup(p => p.Start(WtPath, "cmd", null)).Throws(new InvalidOperationException("boom"));

        var outcome = Launcher().Launch(ShellPath, "pwsh", [Tab("s1"), Tab("s2")], TerminalWindowMode.SingleWindow);
