...
{
    ValueTask<SessionSummary[]> ListRecentAsync(int limit = 20, CancellationToken ct = default);
    ValueTask<SessionSummary[]> ListByRepositoryAsync(string repository, CancellationToken ct = default);
    ValueTask<SessionSummary[]> ListByCwdAsync(string cwd, CancellationToken ct = default);
